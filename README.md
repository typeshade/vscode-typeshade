...
| [`packages/mcp-server`](./packages/mcp-server)             | `@typeshade/mcp`, a Model Context Protocol server. Coding agents (Claude Code, Codex, Cursor, VS Code agent mode, Gemini CLI) get the same diagnostics as the editor, the emitted WGSL and GLSL, types and navigation, the vocabulary, where a GLSL or HLSL spelling gets TypeShade's name, and a CPU run of a shader function. |

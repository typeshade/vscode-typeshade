...
**Fix the first error first, and read each message to its end.** A mistake is reported where it
is made, and a declaration the compiler refused is the one diagnostic for its name: the later
uses of the name add nothing. Where the compiler knows the fix, the message's last sentence is
it: the nearest name for a misspelling (`Did you mean "gain"?`), or TypeShade's name for a GLSL
or HLSL one (`HLSL's lerp is mix here.`).

| Code   | Name                      | Typical cause                                                                                                                                                                                                                                                           | Fix                                                                  |
| ------ | ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------- |
| TS8001 | MISSING_DIRECTIVE         | the file has no `"use typeshade"` directive at all                                                                                                                                                                                                                      | add it as the first line                                             |
...
| TS8017 | SWITCH_CASE               | a case label that is not an integer constant, a repeated label, an empty case above `default:`, a case body that falls through into the next case                                                                                                                       | integer constants; end each case with `break`                        |
...
| TS8022 | UNKNOWN_NAME              | an unknown identifier, field or swizzle (`.st`); a name read before its declaration; an object literal no struct matches; `undefined`                                                                                                                                   | the name the message suggests; declare before use                    |
...
| TS8069 | MISPLACED_DIRECTIVE       | `"use typeshade"` after another top-level statement                                                                                                                                                                                                                     | move it to the first line                                            |
| TS8070 | KERNEL_LOOP_ON_CPU        | a warning on a loop of a kernel function (an exported function taking an array with no size) that the compiler could not prove independent; the message names the line and the names that stop it                                                                       | follow the message's remedy, or accept that the loop runs on the CPU |
| TS8071 | CONSOLE_NOT_RECORDED      | a warning under `console: 'gpu'`: a `console` call the WGSL cannot record (a vertex entry reaches it, an argument has no fixed size, the stage already binds eight storage buffers)                                                                                     | log on the fragment or compute side; the CPU still gets it           |

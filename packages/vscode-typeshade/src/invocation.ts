...
export function spell(type: ShaderType): string {
  switch (type.kind) {
    ...
    case 'f64':
      return 'f64';
    case 'vec64':
      return `vec${type.n}<f64>`;
    case 'vec':
      return `vec${type.n}${suffix(type.elem)}`;
    case 'mat':
      return type.elem === 'f64'
        ? `mat${type.cols}x${type.rows}<f64>`
        : `mat${type.cols}x${type.rows}`;
    case 'struct':
      return type.name;
    case 'array':
      return type.size === undefined
        ? `${spell(type.elem)}[]`
        : `${spell(type.elem)}[${type.size}]`;
    default:
...
/** The letter that follows a vector's dimension for a non-`f32` element. */
...
function check(value: unknown, type: ShaderType): string | undefined {
  switch (type.kind) {
    ...
    case 'mat':
      // A matrix is its columns, each a vector of `rows` components, which is how the oracle's
      // own values are shaped.
      if (!Array.isArray(value) || value.length !== type.cols) {
        return `expected ${type.cols} columns for ${spell(type)}`;
      }
      for (const column of value) {
        const problem = components(column, type.rows, spell(type));
